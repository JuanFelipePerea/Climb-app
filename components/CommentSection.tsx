  ...
  if (photoURL) {
    return (
      <Image
        ...
        width={size}
        height={size}
        style={{ borderRadius: '50%', objectFit: 'cover', flexShrink: 0, alignSelf: 'flex-start', marginTop: '4px' }}
      />
    )
  }
...
function SkeletonComment() {
  return (
    <div style={{ display: 'flex', gap: '14px', padding: '20px 0' }}>
      <div style={{ width: 36, height: 36, borderRadius: '50%', backgroundColor: 'var(--border)', flexShrink: 0, animation: 'skeleton-pulse 1.5s ease-in-out infinite' }} />
...
.cs-textarea:focus { border-color: #4F46E5; }
.cs-submit { width: 100%; }
@media (min-width: 640px) {
  .cs-submit { width: auto; }
}
`
  ...
  return (
    <>
      ...
      <section>
        ...
        {/* Compose area */}
        {authLoading ? (
          <div style={{ height: '100px', borderRadius: '14px', backgroundColor: 'var(--border)', animation: 'skeleton-pulse 1.5s ease-in-out infinite', marginBottom: '32px' }} />
        ) : user ? (
          <div
            style={{
          ...
          >
            <Avatar name={user.displayName ?? user.email ?? 'U'} photoURL={user.photoURL} size={36} />
            <div style={{ flex: 1 }}>
              <textarea
                ref={textareaRef}
                value={text}
          ...
          >
            <textarea
              ...
              className="cs-textarea"
              style={{ cursor: 'not-allowed', color: '#9CA3AF', backgroundColor: '#F9FAFB', display: 'block', marginBottom: '16px' }}
            />
            <div style={{ textAlign: 'center' }}>
              <p style={{ fontFamily: 'Inter, system-ui, sans-serif', fontSize: '14px', color: 'var(--text-mid)', lineHeight: 1.6, marginBottom: '16px' }}>
        ...
        {loadingComments ? (
          ...
          <p style={{ fontFamily: 'Inter, system-ui, sans-serif', fontSize: '15px', color: 'var(--text-light)', textAlign: 'center', padding: '32px 0', lineHeight: 1.6 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column' }}>
            {comments.map((comment, i) => (
              ...
              >
                <Avatar name={comment.userName} photoURL={comment.userPhoto} size={36} />
                <div style={{ flex: 1 }}>
                  <div style={{ display: 'flex', alignItems: 'baseline', gap: '10px', marginBottom: '6px' }}>
                    <span style={{ fontFamily: "'Plus Jakarta Sans', system-ui, sans-serif", fontSize: '14px', fontWeight: 700, color: 'var(--dark)' }}>
                      {comment.userName}
                    </span>
                  ...
                  <p style={{ fontFamily: 'Inter, system-ui, sans-serif', fontSize: '14px', color: 'var(--text-mid)', lineHeight: 1.7, margin: 0 }}>
                    {comment.text}
                  </p>
                </div>
              </div>
